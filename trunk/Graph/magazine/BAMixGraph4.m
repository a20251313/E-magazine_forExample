...
//        CPTScatterPlot *linePlot=[[CPTScatterPlot alloc]init];
        ...
        if(metric.hasY2Axis)
        {
            [self.y2axisArray addObject:[NSString stringWithFormat:@"%d", i]];
                ...
                CPTGradient *gradient=[CPTGradient gradientWithBeginningColor:[BAColorHelper stringToCPTColor:@"26a7e0" alpha:@"1"] endingColor:[BAColorHelper stringToCPTColor:@"07e1ed" alpha:@"1"]];
                gradient.angle=90;
...
-(void)renderAxisInHostView:(CPTGraphHostingView *)hostView
{
    ...
    axisStyle.lineColor=[BAColorHelper stringToCPTColor:@"06557e" alpha:@"1"];
    ...
    for (int i=0; i<tempLabel.count; i++) {
        ...
        CPTAxisLabel *xlabel=[[CPTAxisLabel alloc]initWithText:labelText textStyle:textStyle];
...
//    }
    ...
    yMajorGridLineStyle.lineColor=[BAColorHelper stringToCPTColor:@"03476c" alpha:@"1"];
...
{
    NSMutableArray *dataValues=[tempData objectForKey:(NSString*)barPlot.identifier];
    if (index==[dataValues count]-1) {
        CPTGradient *gradient=[CPTGradient gradientWithBeginningColor:[BAColorHelper stringToCPTColor:@"14e527" alpha:@"1"] endingColor:[BAColorHelper stringToCPTColor:@"83ff8d" alpha:@"1"]];
        gradient.angle=90;
...
-(NSNumber *)numberForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index{
    ...
    switch (fieldEnum) {
        ...
        {
            double data=[[dataValues objectAtIndex:index]doubleValue];
            data=data*(arc4random()%8+5)*0.1;
            
            number=[NSNumber numberWithDouble:data];
            break;
        }

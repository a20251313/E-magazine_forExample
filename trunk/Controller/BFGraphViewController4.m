...
@interface BFGraphViewController4 ()
{
    NSMutableArray *list;
    BADocument *document;
...
@implementation BFGraphViewController4
...
- (IBAction)showMap:(UIButton *)sender {
    DTDocument* dt = [documentMgr getDocumentById:@"000000"];
    DTReport* report = [dt.reports objectAtIndex:0];
    DTReport* report2= [dt.reports objectAtIndex:1];
    
    testViewController* tvc = [[testViewController alloc] initWithDTReport:report];
    
    [tvc setDataReportArray:[[NSArray alloc]initWithObjects:report,report2,nil]];
    //tvc.modalTransitionStyle=UIModalTransitionStyleFlipHorizontal;
    [self presentViewController:tvc animated:YES completion:^(void){}];
}
- (IBAction)showTable:(UIButton *)sender {
    BFTableViewController4 *vc=[[BFTableViewController4 alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([tableView isEqual:_leftTable]) {
        ...
        BFGraph4Cell *cell=[tableView dequeueReusableCellWithIdentifier:@"cell"];
        if (cell==nil) {
            cell=(BFGraph4Cell*)[[[NSBundle mainBundle]loadNibNamed:@"BFGraph4Cell" owner:self options:nil]lastObject];
        }
        cell.textLabel.font=[UIFont boldSystemFontOfSize:14];
        ...
        BAMicroGraph4 *microGraph=[[BAMicroGraph4 alloc]init];
        [microGraph configure:leftModel.salesReport];
        //[microGraph renderInHostView:cell.hostView];
        UIImageView *imageView=[[UIImageView alloc]initWithImage: [microGraph microImage]];
        imageView.frame=cell.hostView.bounds;
        [cell.hostView addSubview:imageView];

        CAGradientLayer *newShadow = [[CAGradientLayer alloc] init];
        CGRect frame=cell.barView.superview.bounds;
        float width=[leftModel.sales doubleValue]/leftModel.maxSales*frame.size.width;
        cell.barView.frame=CGRectMake(0, 0, width, CGRectGetHeight(frame));

        float location=[leftModel.salesTarget doubleValue]/leftModel.maxSales*frame.size.width;
        CGRect flagFrame=cell.flagView.frame;
        flagFrame.origin.x=location;
        cell.flagView.frame=flagFrame;
        
        newShadow.frame = cell.barView.frame;
        if (width>=location) {
            ...
            [cell.botton setBackgroundImage:[UIImage imageNamed:@"graph4-button-green"] forState:UIControlStateNormal];
            
        }else
        {
            newShadow.colors = [NSArray arrayWithObjects:(id)[BAColorHelper stringToUIColor:@"8b0728" alpha:@"1"].CGColor,(id)[BAColorHelper stringToUIColor:@"f92051" alpha:@"1"].CGColor,nil];
            [cell.botton setBackgroundImage:[UIImage imageNamed:@"graph4-button-red"] forState:UIControlStateNormal];
        }
        [cell.botton setTitle:[NSString stringWithFormat:@"%.1f",[leftModel.sales doubleValue]-[leftModel.salesTarget doubleValue] ] forState:UIControlStateNormal];
        [cell.barView.layer addSublayer:newShadow];
        
        CAGradientLayer *pShadow = [[CAGradientLayer alloc] init];
        CGRect pframe=cell.barView2.superview.bounds;
        float pwidth=[leftModel.profit doubleValue]/leftModel.maxProfit*frame.size.width;
        cell.barView2.frame=CGRectMake(0, 0, width, CGRectGetHeight(pframe));
        
        float plocation=[leftModel.profitTarget doubleValue]/leftModel.maxProfit*frame.size.width;
        CGRect pflagFrame=cell.flagView2.frame;
        pflagFrame.origin.x=plocation;
        cell.flagView2.frame=pflagFrame;
        
        pShadow.frame = cell.barView2.frame;
        if (pwidth>=plocation) {
            pShadow.colors = [NSArray arrayWithObjects:(id)[BAColorHelper stringToUIColor:@"536f1d" alpha:@"1"].CGColor,(id)[BAColorHelper stringToUIColor:@"a9d447" alpha:@"1"].CGColor,nil];
            [cell.botton2 setBackgroundImage:[UIImage imageNamed:@"graph4-button-green"] forState:UIControlStateNormal];
            
        }else
        {
            pShadow.colors = [NSArray arrayWithObjects:(id)[BAColorHelper stringToUIColor:@"8b0728" alpha:@"1"].CGColor,(id)[BAColorHelper stringToUIColor:@"f92051" alpha:@"1"].CGColor,nil];
            [cell.botton2 setBackgroundImage:[UIImage imageNamed:@"graph4-button-red"] forState:UIControlStateNormal];
        }
        [cell.botton2 setTitle:[NSString stringWithFormat:@"%.1f%%",([leftModel.profit doubleValue]-[leftModel.profitTarget doubleValue])*100 ] forState:UIControlStateNormal];
        [cell.barView2.layer addSublayer:pShadow];
        ...
        if (([leftModel.sales doubleValue]-[leftModel.salesTarget doubleValue])>=0) {
            cell.salesLabel.textColor=[BAColorHelper stringToUIColor:@"a9d447" alpha:@"1"];
        }else
        {
            cell.salesLabel.textColor=[BAColorHelper stringToUIColor:@"f92051" alpha:@"1"];
        }
        
        if ([leftModel.salesH doubleValue]<0) {
            cell.arrowImageView.image=[UIImage imageNamed:@"graph4-down"];
        }else{
            cell.arrowImageView.image=[UIImage imageNamed:@"graph4-up"];
        }
...
{
    [super viewDidLoad];
    _topScrollView.contentSize=CGSizeMake(1644, _topScrollView.frame.size.height);
    _rightScrollView.contentSize=CGSizeMake(1644, _rightTable.frame.size.height);

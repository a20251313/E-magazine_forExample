...
- (void)calcNodePositions:(CGRect)rect nodes:(NSArray *)nodes width:(CGFloat)width height:(CGFloat)height depth:(NSInteger)depth withCreate:(BOOL)createNode {
    if (nodes.count <= 1) {
        ...
        if (createNode /*|| index >= self.subviews.count*/) {
            ...
            [dicy setValue:cell forKey:[NSString stringWithFormat:@"%d", index]];
...
- (NSArray *)getData {
    ...
    [nodes sortUsingComparator:^(id obj1, id obj2) {
        
        if ([[obj1 objectForKey:@"value"] floatValue] > [[obj2 objectForKey:@"value"] floatValue]) {
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        initialized = NO;
        scaleFactor = 1.0f;
        dicy = [[NSMutableDictionary alloc] init];
    }
    return self;
}
...
- (void)layoutSubviews {
    ...
    if (!initialized) {
        ...
        for (TreemapViewCell* view in self.subviews) {
            [view setScaleFactor:self.scaleFactor];
            [view setNeedsLayout];
        }
    }
...
{
    TreemapViewCell* cell;
    cell = (TreemapViewCell*)[dicy objectForKey:[NSString stringWithFormat:@"%d", index]];
    return cell;
}
